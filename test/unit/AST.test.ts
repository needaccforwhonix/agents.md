...
describe('AST Demock Validation', () => {
  ...
  it('should invalidate code containing any type', () => {
    const code = `function processData(data: any) { return data; }`;
    const result = analyzeCodeBlock(code);
    expect(result.isValid).toBe(false);
    expect(result.errors.some(err => err.includes("Usage of 'any' type detected"))).toBe(true);
  });
