...
describe('AgentMesh E2E Simulation', () => {
  ...
  it('should reject messages with invalid Demock patterns', async () => {
    ...
    // Note: If the test agent does respond, it means the test prompt has "TODO" causing it to be rejected later in the loop.
    // The initial message is tracked, but any responses generated by agents receiving it may get dropped.
    const responsesToInvalidMessage = messages.filter(m => m.senderId === 'test-dev' && m.how === invalidMessage.how);
    expect(responsesToInvalidMessage.length).toBe(0);
  });
});
