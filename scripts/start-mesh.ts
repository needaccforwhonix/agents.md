...
async function startBackgroundMesh() {
  ...
  const mesh = new Mesh(20000); // Higher message limit for longer simulation
  ...
  const initialMessage: Message = {
    ...
    how: "Every a2a agent uses Agentic Context Engineering and AlphaEvolve algorithm with reasoning and full context. Receive every output as input and explicitly decide how to react. Define exactly what, where, and how improvements should be made. Dabei kann gerne stets geholfen werden. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Ensure all changes are additive and non-destructive.",
