...
async function startBackgroundMesh() {
  ...
  const stylesAgent = new Agent("bg-agent-dir-4", "SysStylesBot", "Styles Manager", brain, { responsiveness: 0.1 });
  const publicAgent = new Agent("bg-agent-dir-5", "SysPublicBot", "Public Manager", brain, { responsiveness: 0.1 });
  const testAgent = new Agent("bg-agent-dir-6", "SysTestBot", "Test Manager", brain, { responsiveness: 0.1 });
  ...
  const initialMessage: Message = {
    ...
    timestamp: Date.now(),
    what: "Run continuous background optimization and refactoring pass for logic and prompts",
    where: "All directories (components, pages, scripts, test, styles, public)",
    how: "Use AlphaEvolve and Agentic Context Engineering to propose long-term improvements to code logic, prompts, and their implementations",
