...
async function startBackgroundMesh() {
  ...
  const initialMessage: Message = {
    ...
    timestamp: Date.now(),
    what: "Run continuous background optimization and refactoring pass across the Agent2Agent structure where everything gets its own a2a agent. All agents receive every output as input and can decide how to react. Every a2a agent uses Agentic context engineering and alphaevolve Algorithmus with reasoning and full context. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne steht's geholfen werden.",
