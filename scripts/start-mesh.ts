...
async function startBackgroundMesh() {
  ...
  const mesh = new Mesh(50000); // Much higher message limit for a complete, longer simulation
  ...
  const initialMessage: Message = {
    ...
    timestamp: Date.now(),
    what: "Run continuous background optimization and refactoring pass across the Agent2Agent structure where everything gets its own a2a agent. Actively propose improvements emphasizing Sicherheit Performance Style documentation Sauberkeit Ordnung. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist.",
    ...
    how: "Every a2a agent uses Agentic context engineering and alphaevolve Algorithmus with reasoning and full context. Receive every Output as Input and can decide how to react. Define exactly what, where, and how improvements should be made. Dabei kann gerne steht's geholfen werden. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Ensure all changes are strictly additive and non-destructive.",
