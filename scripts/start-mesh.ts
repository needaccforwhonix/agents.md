...
async function startBackgroundMesh() {
  ...
  const mesh = new Mesh(100000); // Higher message limit for much longer, deeper simulation runs
  ...
  const initialMessage: Message = {
    ...
    timestamp: Date.now(),
    what: "(WAS) Führe eine kontinuierliche Hintergrundoptimierung und Refactoring in der Agent2Agent-Struktur durch, bei der jede Domäne, jedes Verzeichnis und jede Datei einen eigenen dedizierten Agenten erhält. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne stets geholfen werden. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben. Sicherheit Performance Style documentation Sauberkeit Ordnung. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Keine Deletionen von Features, Ideen oder Funktionen! Alles ist additiv.",
    where: "(WO) Alle dynamisch registrierten Dateien, Verzeichnisse und Konfigurationsknoten im gesamten Projekt. Zielorte müssen eindeutig spezifiziert werden, um asynchron parallel alles weiter zu entwickeln und aktuell zu bleiben.",
    how: "(WIE) Jeder A2A-Agent nutzt Agentic Context Engineering und den AlphaEvolve-Algorithmus mit Reasoning und vollständigem Kontext. Empfange jeden Output als Input. Definiere genau wie Verbesserungen vorgenommen werden. Vollständig optimiert: Demock - Testing - ACE - CI/CD - E2E - AST - Documentationen - Todos. Sicherheit Performance Style documentation Sauberkeit Ordnung. Ausgabe exakt als: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements].",
    reasoning: "(WARUM) Um eine robuste Agent2Agent-Struktur zu stärken, die eine massive parallele asynchrone Evolution ermöglicht, um kontinuierlich aktualisiert, optimiert (Sicherheit Performance Style documentation Sauberkeit Ordnung) und sauber organisiert zu bleiben.",
  };
