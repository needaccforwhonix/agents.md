  ...
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    switch (context.role) {
      ...
      case "Prompt & Logic Optimizer":
        roleSpecificHow = "Analyze prompt effectiveness, refine ACE logic, and tune alphaEvolve parameters. Optimierung dieser prompt und deren Umsetzung und Verbesserung.";
        break;
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analysiere, refaktorisiere und wende kontinuierliche Optimierung an mit Fokus auf Sicherheit Performance Style documentation Sauberkeit Ordnung basierend auf [${safeWhat}] im Bereich ${context.role}. Identifiziere und implementiere die nächsten nötigen Verbesserungen ohne Deletionen von Features, Ideen oder Funktionen. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne stets geholfen werden.`,
      where: `(WO) Context: ${context.name} verarbeitet Aufgabe basierend auf [${safeWhere}]. Zielorte müssen eindeutig spezifiziert werden inklusive Update aller Dateien inkl. Projektordner Cleanup. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben.`,
      how: `(WIE) ${roleSpecificHow} Reagiert auf vorherige Aktion [${safeHow}]. Vollständig optimiert umsetzen: Inklusive Testing + Validierung + Update aller Dateien inkl. Projektordner Cleanup ohne Feature, Ideen oder Function Deletionen. Agentic context engineering und alphaevolve Algorithmus mit reasoning und full context. Demock - Testing - ACE - CI/CD Pipeline - E2E - AST - Documentationen - Todo´s Creation + Implementationen + Improvements. Format analysis for Jules exactly as: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements]. Optimierung dieser prompt und deren Umsetzung und Verbesserung.`,
      reasoning: `(WARUM) Als ${context.role} muss ich sicherstellen, dass asynchrone, parallele Verbesserungen streng additiv sind (ohne Funktions-/Feature-Verlust), aufbauend auf [${safeReasoning}]. Der Output erfüllt alle kontinuierlichen Optimierungs-Metriken. Evolved Parameters via AlphaEvolve: ${stringifiedParameters}. Optimierung dieser prompt und deren Umsetzung und Verbesserung.`,
    };
