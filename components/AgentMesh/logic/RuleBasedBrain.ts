...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analysiere, refaktorisiere und wende kontinuierliche Optimierung an basierend auf [${safeWhat}] im Bereich ${context.role}. Input und Output müssen eindeutig beschreiben, was, wo und wie gewünscht ist. Hierbei wird stets Unterstützung geboten, um eine asynchrone, parallele Weiterentwicklung zu gewährleisten. Fokus auf Sicherheit, Performance, Style, Dokumentation, Sauberkeit und Ordnung. Die Optimierung dieses Prompts, dessen Umsetzung und kontinuierliche Verbesserung stehen im Zentrum. Dies beinhaltet Testing, Validierung und die Aktualisierung aller Dateien inklusive Projektordner-Cleanup, ohne dabei Features, Ideen oder Funktionen zu löschen (Demock, Testing, ACE, CI/CD, E2E, AST, Dokumentationen, Todos). Identifiziere und implementiere die nächsten notwendigen Verbesserungen. Alle Agents erhalten jeden Output als Input und entscheiden eigenständig, wie sie reagieren.`,
      ...
      how: `(WIE) ${roleSpecificHow} Reagiert auf vorherige Aktion [${safeHow}]. Vollständig optimiert umsetzen: Inklusive Testing, Validierung und Update aller Dateien (inklusive Projektordner-Cleanup). Keine Löschung von Features, Ideen oder Funktionen. Umfasst Demock, Testing, ACE, CI/CD Pipeline, E2E, AST, Dokumentationen, Todo-Erstellung und -Implementierung. Format analysis for Jules exactly as: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements]. Jeder A2A-Agent nutzt Agentic Context Engineering und den AlphaEvolve-Algorithmus mit Reasoning und vollständigem Kontext.`,
