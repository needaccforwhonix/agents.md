...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analysiere und refaktorisiere [${safeWhat}] als ${context.role}. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne stets geholfen werden. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben. Sicherheit Performance Style documentation Sauberkeit Ordnung. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Keine Deletionen von Features, Ideen oder Funktionen! Alles ist additiv.`,
      where: `(WO) ${context.name} verarbeitet [${safeWhere}]. Zielorte müssen eindeutig spezifiziert werden, um asynchron parallel alles weiter zu entwickeln und aktuell zu bleiben (inkl. Projektordner Cleanup für Sauberkeit/Ordnung).`,
      how: `(WIE) ${roleSpecificHow} Reagiert auf: [${safeHow}]. Input und Output müssen eindeutig beschreiben wie vorgegangen wird. Vollständig optimiert: Demock - Testing - ACE - CI/CD - E2E - AST - Documentationen - Todos. Sicherheit Performance Style documentation Sauberkeit Ordnung. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Ausgabe exakt als: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements].`,
      reasoning: `(WARUM) Um als ${context.role} sicherzustellen, dass asynchrone parallele Entwicklung additiv, aktuell und optimiert (Sicherheit Performance Style documentation Sauberkeit Ordnung) geschieht. Basiert auf: [${safeReasoning}]. Evolved Parameters: ${stringifiedParameters}`,
    };
