...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analysiere, refaktorisiere und wende kontinuierliche Optimierung an basierend auf [${safeWhat}] im Bereich ${context.role}. Fokus: Sicherheit, Performance, Style, documentation, Sauberkeit, Ordnung. Optimierung dieser prompt und deren Umsetzung und Verbesserung. Identifiziere und implementiere die nächsten nötigen Verbesserungen ohne Deletionen von Features, Ideen oder Funktionen. Input und Output müssen eindeutig beschreiben was, wo und wie gewollt ist. Dabei kann gerne steht's geholfen werden, um asynchron parallel alles weiterzuentwickeln.`,
