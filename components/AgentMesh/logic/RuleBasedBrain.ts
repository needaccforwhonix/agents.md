...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analyze, validate, and apply continuous optimization targeting Sicherheit, Performance, Style, documentation, Sauberkeit, and Ordnung based on [${safeWhat}] adhering to strict additive-only policies in the domain of ${context.role}. Identify and implement the next necessary improvements. Input und Output müssen eindeutig beschreiben was, wo und wie gewollt ist.`,
      ...
      how: `(WIE) ${roleSpecificHow} Reacting to previous action [${safeHow}]. Ensure comprehensive integration of Demock testing, ACE, CI/CD pipeline checks, E2E testing, AST validations, Documentation updates, and Todo creation/implementation. Format analysis for Jules exactly as: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements].`,
      reasoning: `(WARUM) As a ${context.role}, I must ensure asynchronous, parallel improvements are non-destructive and strictly additive. Output strictly aligns with continuous optimization metrics, AST Demock standards, ACE boundaries, and E2E validation. Evolved Parameters via AlphaEvolve: ${stringifiedParameters}`,
