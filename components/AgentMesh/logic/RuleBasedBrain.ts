...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      where: `(WO) Context: ${context.name} processing task originating from [${safeWhere}]. Targeted locations must be clearly specified. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben.`,
      how: `(WIE) ${roleSpecificHow} Reacting to previous action [${safeHow}]. Ensure comprehensive integration of Demock testing, ACE, CI/CD pipeline checks, E2E testing, AST validations, Documentation updates, and Todo creation/implementation. Format analysis for Jules exactly as: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements]. Dabei kann gerne steht's geholfen werden. Optimierung dieser prompt und deren Umsetzung und Verbesserung.`,
      reasoning: `(WARUM) As a ${context.role}, I must ensure asynchronous, parallel improvements are non-destructive and additive, building upon previous reasoning [${safeReasoning}]. The output strictly adheres to continuous optimization metrics: Sicherheit Performance Style documentation Sauberkeit Ordnung. Evolved Parameters via AlphaEvolve: ${stringifiedParameters}`,
    };
