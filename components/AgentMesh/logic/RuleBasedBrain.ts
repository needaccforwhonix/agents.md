...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      senderId: context.id,
      timestamp: Date.now(),
      what: `(WAS) Analysiere, refaktorisiere und wende kontinuierliche Optimierung an mit Fokus auf Sicherheit, Performance, Style, documentation, Sauberkeit und Ordnung basierend auf [${safeWhat}] im Bereich ${context.role}. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne stets geholfen werden. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben.`,
      where: `(WO) Context: ${context.name} verarbeitet Aufgabe basierend auf [${safeWhere}]. Zielorte müssen eindeutig spezifiziert werden inklusive Update aller Dateien inkl. Projektordner Cleanup.`,
      how: `(WIE) ${roleSpecificHow} Reagiert auf vorherige Aktion [${safeHow}]. Alle Agenten empfangen jeden Output als Input und entscheiden, wie sie reagieren. Nutze Agentic Context Engineering und AlphaEvolve Algorithmus mit reasoning und full context. Optimierung dieser Prompt und deren Umsetzung und Verbesserung.`,
      reasoning: `(WARUM) Use a agent2agent structure where everything gets ist own a2a agent. All agents receive every Output as Input and can decide how to react. Als ${context.role} basierend auf [${safeReasoning}]. Evolved Parameters via AlphaEvolve: ${stringifiedParameters}`,
    };
