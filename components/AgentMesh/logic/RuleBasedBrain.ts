...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = {
      ...
      how: `(WIE) [EXECUTION STRATEGY] ${roleSpecificHow} Reagiert auf [${safeHow}]. Vollständig optimiert umsetzen durch strikte Einhaltung der Checklist: Demock, Testing (E2E/Unit), ACE, CI/CD Pipeline Checks, AST Validierung, Dokumentation und Todo Management. Format analysis exactly as: @jules [Root Cause Analysis des Problems (Include your CoT here)] + [Actionable Code-Fix] + [Quality/Performance Improvements]. Input und Output müssen eindeutig beschreiben was wo wie gewollt ist. Dabei kann gerne stehts geholfen werden. So soll asynchron parallel alles weiter entwickelt werden und aktuell bleiben. Sicherheit Performance Style documentation Sauberkeit Ordnung. Optimierung dieser prompt und deren Umsetzung und Verbesserung.`,
