...
import { promptBuilder } from "./AlphaEvolve";
...
export class RuleBasedBrain implements Brain {
  async decide(message: Message, context: AgentContext): Promise<Message | null> {
    ...
    const response: Message = promptBuilder(context, { safeWhat, safeWhere, safeHow, safeReasoning, roleSpecificHow, stringifiedParameters });
