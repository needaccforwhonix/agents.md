...
import { ASTAnalysisResultV2 } from "./Types";
  ...
  const visit = (node: ts.Node) => {
    ...
    if (ts.isPropertyAccessExpression(node)) {
      ...
      if (expressionText === "console" && nameText === "log") {
        warnings.push("Optimization Warning: Usage of console.log() detected. Consider removing console.log calls in production code.");
      }
    }
